...
- 请求支持 application/json 和 multipart/form-data 两种。对于类型为 multipart/form-data 的请求，当字段类型为对象或数组时需要将字段序列化为 JSON 字符串后进行调用，可参考下文的 示例 。
...
| 字段名 | 类型 | 描述 |
| --- | --- | --- |
| content | string | 选填，消息内容，文本内容，支持[内嵌格式](%E5%86%85%E5%B5%8C%E6%A0%BC%E5%BC%8F.md) |
...
| msg\_id | string | 选填，要回复的消息id([Message](/wiki/develop/api-v2/server-inter/message/template/model.html#message).id), 在 [AT\_MESSAGE\_CREATE](%E9%A2%91%E9%81%93%E6%B6%88%E6%81%AF%E4%BA%8B%E4%BB%B6.md#at_message_create) 事件中获取。 |
...
- 主动消息：发送消息时，未填充 msg\_id/event\_id 字段的消息。
- 被动消息：发送消息时，填充了 msg\_id/event\_id 字段的消息。 msg\_id 和 event\_id 两个字段任意填一个即为被动消息。接口使用此 msg\_id/event\_id 拉取用户的消息或事件，同时判断用户消息或事件的发送时间，如果超过被动消息回复时效，将会不允许发送该消息。
